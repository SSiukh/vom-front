...
- **Content scale** (`data/content-scales.ts`, select "Масштаб вмісту"): XS 0.7 / S 0.85 / **M 1 (default = the standard fit inside the 16.6 % padding)** / L 1.1 / XL 1.2, applied to icon + text together (`contentScale` option of `layoutSticker`). L and XL enlarge past the standard fit but are clamped so the composition never comes closer than `MIN_PADDING_TO_HEIGHT_RATIO` (8 % of the height) to any edge, so long text stops growing at the edge; the composition stays centred. The factors are the assistant's choice, tuned once on the user's feedback ("XS not that small, XL larger, M standard").
...
## Original-colour icons (`instagram-color`, `tiktok-color`)

Icons are a union: `kind: 'mono'` (paths, take the artwork colour) or `kind: 'color'` (layers with their own paint + gradients + a fixed `textPaint`). Colour icons never take the artwork colour: Instagram text is a linear gradient (#FA8F21 → #D82D7E → #8C3AAA, bottom-left → top-right across the text box), TikTok text is black; the "Колір малюнка" control is disabled and hidden, the background stays selectable, and the contrast warning uses `textPaint` (black / `contrastColor`).

- `layoutSticker` returns `layers` (`{d, fill}`, `fill: null` = artwork colour) and `gradients`. Icon radial gradients are `userSpaceOnUse`, so their matrix is carried through the icon's scale and shift; ids are `<icon>-<key>-<hash of the scaled matrix>` (equal id ⇒ identical definition, which is what makes several inline SVGs on one page safe). The text gradient uses `objectBoundingBox` units and the fixed id `<icon>-text`; if a per-instance parameter is ever added to it, extend the id.
- `exportSvg` writes `<defs>` plus a `fill` on every path and validates ids, `#rrggbb` stops, finite numbers and `url(#id)` fills. `<app-sticker-svg>` renders the same document inline (preview and mock-up cards).
- Adding a colour icon: a `kind: 'color'` entry in `sticker-icons.ts` (gradients from the source SVG: `cx/cy/r` + `gradientTransform` as a 6-number matrix), extend `StickerIconId`; the data and layout specs run over it.
...
- **A font**: put a static TTF/OTF/WOFF (never WOFF2, never variable) and its licence into `public/fonts/<Family>/`; add `{ id, label, url }` to `STICKER_FONTS` with a **relative** URL (`fonts/...`, no leading slash — respects `<base href>`). Check coverage first; the data spec forbids absolute URLs and unreadable formats. Jua (supplied) covers Basic Latin, digits, Hangul and a few symbols — **no Cyrillic**; the page lists any character missing from the chosen font and blocks download rather than drawing `.notdef` boxes. A Cyrillic font must contain `ї є і ґ`. Jua is 2.1 MB (2,520 glyphs, mostly Hangul); the chosen font is fetched when selected (Jua on page open). **Nunito ExtraBold** (`public/fonts/Nunito/`, 125 kB, OFL) is the Cyrillic option: full Ukrainian + Latin verified via cmap, glyf outlines + GPOS. Google Fonts serves Nunito only as a variable file in the repo; the static instance came from the Fonts CSS API with an old user agent (`curl -A "Mozilla/4.0" https://fonts.googleapis.com/css2?family=Nunito:wght@800`) — vendor the file, gstatic URLs are versioned. Subsetting to Latin would cut it to a few dozen kB — OFL allows it and no Reserved Font Name is declared, but it modifies the user's file, so ask first.
